...
class CMUNeXt_LLM(nn.Module):
    def __init__(self, unfreeze=False, need_init=False, mode="llama", input_channel=3, num_classes=1, dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
        """
        Args:
            input_channel : input channel.
            num_classes: output channel.
            ...
            kernels: kernal size of cmunext blocks
        """
        super(CMUNeXt_LLM, self).__init__()
        # Encoder
        self.Maxpool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.stem = conv_block(ch_in=input_channel, ch_out=dims[0])
        self.encoder1 = CMUNeXtBlock(ch_in=dims[0], ch_out=dims[0], depth=depths[0], k=kernels[0])
        ...
        self.llm4seg = LLM4Seg(unfreeze=unfreeze, need_init=need_init, mode=mode, channel=dims[4], layer=14, h=16, w=16)
...
def cmunext_llm(unfreeze, need_init, mode="llama", dims=[16, 32, 128, 160, 256], depths=[1, 1, 1, 3, 1], kernels=[3, 3, 7, 7, 7]):
    return CMUNeXt_LLM(unfreeze=unfreeze, need_init=need_init, mode=mode, dims=dims, depths=depths, kernels=kernels)
